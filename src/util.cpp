...
int nAnonymizefictecpagosAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "fictecpagos" is a composite category enabling all fictecpagos-related debug output
            if (ptrCategory->count(string("fictecpagos"))) {
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty fictecpagos.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "fictecpagosd.pid"));
