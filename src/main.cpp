...
#if defined(NDEBUG)
#error "fictecpagos cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from fictecpagos
...
{
    RenameThread("fictecpagos-scriptch");
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert FTP to zphr
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/fictecpagos Core:1.1.0/" ||
            pfrom->cleanSubVer == "/fictecpagos Core:1.3.0/" ||
            pfrom->cleanSubVer == "/fictecpagos Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // fictecpagos: We use certain sporks during IBD, so check to see if they are
